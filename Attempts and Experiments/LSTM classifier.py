...
train_val_number = int(len(X)*7/8);
...
class GRUClassifier(torch.nn.Module):
    def __init__(self, emb_size, hidden_size, num_layers, num_class):
        ...
        # single layer GRU
        self.lstm = nn.LSTM(emb_size, hidden_size, num_layers = 1, bidirectional = False, batch_first = True);
...
#for i in range(1):
    ...
    for X, y, lens in data_loader:
        ...
        # compute the loss
        loss = nn.functional.cross_entropy(out[0], y, weight = class_weights);
    ...
    with torch.no_grad(): # turn of autograd to make everything faster
        ...
        for X, y, lens in valid_data_loader:
            ...
            chosen = torch.argmax(out[0], dim=1);
